...
from app.routers import learn_portal_auth, learn_portal_dashboard, learn_portal_competences, learn_portal_formations, learn_portal_informations, learn_portal_lms
...
app_local = FastAPI(title="Novoskill - Portail Learn API")
...
@app_local.api_route("/learn/healthz", methods=["GET", "HEAD"])
def healthz():
    return {"status": "ok"}


@app_local.get("/learn/me")
def learn_me(request: Request):
...
@app_local.get("/learn/me/scope")
def learn_me_scope(request: Request):
    auth = request.headers.get("Authorization", "")
    u = learn_require_user(auth)
    ...
    with get_conn() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            ...
            if not profiles and not u.get("is_super_admin"):
                raise HTTPException(
                    status_code=403,
                    detail="Compte non rattaché à un profil Learn.",
                )
...
for route in learn_portal_auth.router.routes:
...
for route in learn_portal_lms.router.routes:
    app_local.router.routes.append(route)
