(function () {
  ...
  window.portal.registerMenu({
    view: "dashboard",
    htmlUrl: "/menu_learn/learn_dashboard.html"
  });

  window.portal.registerMenu({
    view: "informations",
    htmlUrl: "/menu_learn/learn_informations.html"
  });
  ...
  function buildPortalUrlWithId(effectifId) {
    return `/learn/?id=${encodeURIComponent(effectifId)}`;
  }
  ...
  async function tryFillTopbar() {
    ...
    try {
      ...
      const rMe = await fetch(`${API_BASE}/learn/me`, {
      ...
      const rScope = await fetch(`${API_BASE}/learn/me/scope`, {
        headers: { "Authorization": `Bearer ${token}` }
      });
      ...
      if (rScope.ok && scope && Array.isArray(scope.profiles) && scope.profiles.length) {
        ...
        if (name) name.textContent = fullName || "Learn";
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    try { await (window.__learnAuthReady || Promise.resolve(null)); } catch (_) {}
    tryFillTopbar();
  });
})();