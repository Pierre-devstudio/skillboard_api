(function () {
  ...
  function setSsoStatus(elId, status) {
    ...
    const s = (status || "desactive").toString().trim().toLowerCase();
    el.dataset.status = s; el.textContent = ssoStatusLabel(s);
  }
  function setCheckboxValueById(id, value) { const el = byId(id); if (el) el.checked = !!value; }
  function getCheckboxValueById(id) { return !!byId(id)?.checked; }
  function renderClientConnexion(cfg) {
    const c = cfg || {}; _initialConnexion = { ...c };
    setSsoStatus("ficheSsoStatus", c.sso_statut || "desactive");
    setCheckboxValueById("ficheSsoEnabled", c.sso_enabled);
    setCheckboxValueById("ficheSsoPasswordAllowed", c.password_allowed !== false);
    setCheckboxValueById("ficheSsoMandatory", c.sso_obligatoire);
    setInputValue("ficheSsoDomain", c.domaine_autorise);
    setInputValue("ficheSsoProviderId", c.provider_id_supabase);
    setInputValue("ficheSsoMetadataUrl", c.metadata_url);
    setInputValue("ficheSsoEmailAttr", c.attribut_email || "email");
  ...
  function readConnexionPayload() {
    const mandatory = getCheckboxValueById("ficheSsoMandatory");
    return {
      sso_enabled: getCheckboxValueById("ficheSsoEnabled"), domaine_autorise: inputValue("ficheSsoDomain") || null,
      type_sso: "saml_2_0", metadata_url: inputValue("ficheSsoMetadataUrl") || null, metadata_xml: inputValue("ficheSsoMetadataXml") || null,
      provider_id_supabase: inputValue("ficheSsoProviderId") || null, attribut_email: inputValue("ficheSsoEmailAttr") || "email",
      password_allowed: mandatory ? false : getCheckboxValueById("ficheSsoPasswordAllowed"), sso_obligatoire: mandatory,
    };
  }
  function sameConnexionPayload(a, b) {
    const keys = ["sso_enabled", "domaine_autorise", "type_sso", "metadata_url", "metadata_xml", "provider_id_supabase", "attribut_email", "password_allowed", "sso_obligatoire"];
    return keys.every(k => String((a && a[k]) ?? "") === String((b && b[k]) ?? ""));
  }
  async function loadClientConnexion(token) {
    const ownerId = getOwnerId(); const clientId = getClientId();
    if (!ownerId || !clientId || !token) return null;
    try { return await apiJson(`${API_BASE}/studio/connexions/client/${encodeURIComponent(ownerId)}/${encodeURIComponent(clientId)}`, token); }
    catch (e) { return { schema_ready: false, sso_statut: "desactive", test_message: e?.message || "Configuration SSO indisponible." }; }
  }
  async function saveClientConnexionIfChanged(token) {
    const ownerId = getOwnerId(); const clientId = getClientId();
    if (!ownerId || !clientId || !token) return null;
    const payload = readConnexionPayload();
    const initial = _initialConnexion || {};
    const normalizedInitial = {
      sso_enabled: !!initial.sso_enabled, domaine_autorise: initial.domaine_autorise || null, type_sso: initial.type_sso || "saml_2_0",
      metadata_url: initial.metadata_url || null, metadata_xml: initial.metadata_xml || null, provider_id_supabase: initial.provider_id_supabase || null,
      attribut_email: initial.attribut_email || "email", password_allowed: initial.password_allowed !== false, sso_obligatoire: !!initial.sso_obligatoire,
    };
    if (sameConnexionPayload(payload, normalizedInitial)) return initial;
    const saved = await apiJson(`${API_BASE}/studio/connexions/client/${encodeURIComponent(ownerId)}/${encodeURIComponent(clientId)}`, token, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    renderClientConnexion(saved || {});
    return saved;
  }
  async function testClientConnexion() {
    const ownerId = getOwnerId(); const clientId = getClientId(); const token = await getStudioAccessToken();
    const btn = byId("btnFicheTestConnexion"); const old = btn ? btn.textContent : "";
    if (!ownerId || !clientId || !token) return;
    try {
...
function bindPostalAssist(){
  ...
  function getOrgToggleSvg(){
    return `
      <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-29db5b5f37d6"></use></svg>
    `;
  }

  function getOrgPlusSvg(){
    return `
      <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-7ad460c05382"></use></svg>
    `;
  }
  ...
  function renderIdentification(){
    ...
    renderClientConnexion(_initialConnexion || {});
  }
  ...
  function setFicheEditMode(enabled){
      ...
      const controls = document.querySelectorAll(".cs-form-ctrl, .cs-connexion-ctrl, #ficheGroupOk, #ficheTeteGroupe");
      controls.forEach(el => {
  ...
  function bindFicheActions(){
    ...
    bindSsoHelpButtons();
    byId("btnFicheTestConnexion")?.addEventListener("click", async () => {
      await testClientConnexion();
    });
  ...
  async function loadData(){
    ...
    _initialConnexion = await loadClientConnexion(token);
