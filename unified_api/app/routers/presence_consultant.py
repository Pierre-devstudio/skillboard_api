...
from app.routers.MailManager import send_absent_mail
...
# INIT
# ---------------------------------------------------
@app_local.get("/presence_consultant/init")
...
# VALIDATION
# ---------------------------------------------------
@app_local.post("/presence_consultant/validate")
def validate_consultant(
    payload: ConsultantValidationInput,
    request: Request,
        ...
        with get_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                if cur.fetchone()["deja_signe"] > 0:
                    raise HTTPException(
                        status_code=409,
                        detail="Signature déjà enregistrée."
                    )
