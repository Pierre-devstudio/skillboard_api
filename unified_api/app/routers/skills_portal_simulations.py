...
def _build_state(dataset: Dict[str, Any], hypotheses: List[SimulationHypothese], *, simulated: bool) -> Dict[str, Any]:
    ...
    if simulated:
        for h in hypotheses or []:
            ...
            elif h_type == "mobilite_effectif" and eid and eid in effectifs:
            ...
            elif h_type == "montee_competence" and eid:
                cid = str(h.id_comp or "").strip()
                niv = _level_label(h.niveau_simule)
                if cid and niv != "—":
                    # Si la ligne n’existe pas, on la simule. Le réel reste intact, évidemment.
                    comp = next((c for c in dataset.get("competences") or [] if str(c.get("id_comp") or "") == cid), None) or {}
                    skills[(eid, cid)] = {
                        "id_effectif": eid,
                        "id_comp": cid,
                        "niveau_actuel": niv,
                        "code": comp.get("code") or "",
                        "intitule": comp.get("intitule") or "",
                        "domaine": comp.get("domaine") or "",
                    }

            elif h_type == "recrutement_virtuel":
                target = str(h.id_poste_cible or h.id_poste or "").strip()
                if target:
                    virtual_index += 1
                    ...
                    effectifs[veid] = {
                        "id_effectif": veid,
                        "prenom_effectif": "Profil",
                        "nom_effectif": f"virtuel {virtual_index}",
                        "id_poste_actuel": target,
                        "intitule_poste": "Profil recruté / renfort",
                        "codif_poste": "",
                        "nom_service": "",
                        "is_virtual": True,
                    }
                    for req in dataset.get("requirements") or []:
                        if str(req.get("id_poste") or "") == target:
                            ...
                            skills[(veid, cid)] = {
                                "id_effectif": veid,
                                "id_comp": cid,
                                "niveau_actuel": _level_label(req.get("niveau_requis")),
                                "code": req.get("code") or "",
                                "intitule": req.get("intitule") or "",
...
def _compute_impacts(current: List[Dict[str, Any]], simulated: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {"postes_securises": secured, "postes_degrades": degraded, "postes_impactes": items[:30]}
    ...
    try:
        with get_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                dataset = _fetch_simulation_dataset(cur, id_ent, scope.id_service, int(criticite_min))
                payload = _options_payload(dataset)
                payload["scope"] = scope.dict()
    ...
    try:
        with get_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                current_summary = _compute_summary(current_records)
                simulated_summary = _compute_summary(simulated_records)
                impacts = _compute_impacts(current_records, simulated_records)
