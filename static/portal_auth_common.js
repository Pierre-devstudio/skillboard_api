...
(function () {
  ...
  async function signInWithEnterpriseSSO(options) {
    const client = getClient();
    const o = options || {};
    const domain = (o.domain || "").toString().trim().toLowerCase();
    const providerId = (o.providerId || "").toString().trim();
    const redirectTo = (o.redirectTo || "").toString().trim();
    ...
    const { data, error } = await client.auth.signInWithSSO(payload);
    if (error) throw new Error(error.message || "Connexion SSO impossible.");
    return data || {};
  }
  ...
  AuthCommon.signInWithEnterpriseSSO = signInWithEnterpriseSSO;
