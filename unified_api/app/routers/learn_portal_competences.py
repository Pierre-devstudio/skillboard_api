...
def _run_ai_draft(cur, oid: str, payload: AiDraftCompetencePayload) -> dict:
    ...
    sys = (
        ...
        "Une compétence Novoskill est une capacité mobilisable, construite à partir de savoirs, savoir-faire et savoir-être, qui permet de réaliser des tâches utiles aux activités métier ou pédagogiques. "
        "L’intitulé commence toujours par un verbe d’action. Interdiction de produire un thème vague ou une tâche isolée déguisée en compétence. "
        "La fiche compétence doit rester exploitable dans le même catalogue que Studio et donc réutilisable ensuite sur des postes. "
