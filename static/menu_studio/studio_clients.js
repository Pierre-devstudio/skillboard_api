(function () {
  ...
  function getCommercialIconSvg(){
    return `
      <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-aa01548d5592"></use></svg>
    `;
  }
  ...
  function normalizeCommercialOfferCode(value){
  ...
  function getSelectedCommercialOffer(){
    ...
    const norm = normalizeCommercialOfferCode(code);
    return (_commercialOffers || []).find(x => (x.offer_code || "") === code)
      || (_commercialOffers || []).find(x => normalizeCommercialOfferCode(x.offer_code_normalized || x.offer_code || x.offer_label) === norm)
      || null;
  }
  ...
  function renderCommercialOfferOptions(selectedCode){
    ...
    if (selectedCode && sel.value !== selectedCode) {
      const selectedNorm = normalizeCommercialOfferCode(selectedCode);
      const fallback = Array.from(sel.options).find(opt => normalizeCommercialOfferCode(opt.dataset.offerNormalized || opt.value || opt.textContent) === selectedNorm);
