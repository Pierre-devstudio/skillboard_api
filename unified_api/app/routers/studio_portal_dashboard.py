...
def _studio_fetch_referentiel_quality(cur, id_owner: str, id_ents: List[str], id_service: Optional[str] = None) -> Dict[str, Any]:
    ...
    if _studio_has_table(cur, "tbl_competence") and not id_service:
        cur.execute(
            """
...
def _studio_scope_options(current: Dict[str, Any], services: List[Dict[str, Any]], has_linked: bool) -> List[Dict[str, str]]:
    opts = [{"value": "ma_structure", "label": "Ma structure"}]
    for s in services:
        sid = _studio_s(s.get("id_service"))
        if not sid:
            continue
        indent = "— " * min(_studio_i(s.get("depth")), 4)
        opts.append({"value": f"service:{sid}", "label": f"{indent}{s.get('nom_service') or 'Service'}"})
    if has_linked:
        opts.append({"value": "ma_structure_et_liees", "label": "Ma structure + organisations liées"})
        opts.append({"value": "liees", "label": "Organisations liées uniquement"})
    return opts
    ...
    try:
        ...
        with get_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                if requested_perim == "liees" and has_linked:
                    main = _studio_build_main(cur, oid, current, "ma_structure", prio, criticite)
                    main["scope_label"] = "Structure courante affichée en synthèse. Le bloc de supervision ci-dessous porte les organisations liées."
                    linked = _studio_build_linked(cur, oid, linked_structures, prio, criticite)
                else:
                    main_perim = requested_perim if requested_perim.startswith("service:") else "ma_structure"
                    main = _studio_build_main(cur, oid, current, main_perim, prio, criticite)
                    linked = _studio_build_linked(cur, oid, linked_structures, prio, criticite) if has_linked and requested_perim in ("ma_structure_et_liees", "liees") else {"visible": False, "portfolio": {"structures_total": len(linked_structures)}, "structures_prioritaires": [], "actions_prioritaires": []}
