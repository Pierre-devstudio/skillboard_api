(function () {
  ...
  function getRedirectUrlForReset() {
    return `${window.location.origin}/learn_reset_password.html`;
  }
  ...
  async function doLogin() {
    ...
    try {
      ...
      if (!effectifId) {
        setMsg(
          "Connexion OK, mais ce compte nâ€™est pas rattachÃ© Ã  un profil Learn (id_effectif manquant).",
          "error"
        );
        return;
      }

      window.location.href = `/learn/?id=${encodeURIComponent(effectifId)}`;
    } catch (e) {
