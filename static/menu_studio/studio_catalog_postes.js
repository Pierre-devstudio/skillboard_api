(function () {
  ...
  function iconSvg(kind){
    if (kind === "edit") {
      return `<svg viewBox="0 0 24 24" aria-hidden="true" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-job"></use></svg>`;
    }
    if (kind === "archive") {
  ...
  function renderList(){
    ...
    _items.forEach(it => {
      ...
      scope.className = "sb-badge sb-badge--poste-soft studio-catalog-scope";
      ...
      if (isSupervisor()) {
        ...
        btnEdit.setAttribute("aria-label", "Modifier la fiche de poste");
        ...
        if (it.actif) {
          ...
          btnArch.setAttribute("aria-label", "Archiver la fiche de poste");
