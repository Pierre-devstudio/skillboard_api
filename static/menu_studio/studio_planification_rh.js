(function () {
    ...
    return `
      ...
      <div class="studio-rh-pagination" aria-label="Pagination événements RH">
        <button type="button" class="sb-icon-btn studio-rh-page-nav" data-plan-page-nav="prev" title="Page précédente" aria-label="Page précédente"${prevDisabled}>
          <svg viewBox="0 0 24 24" aria-hidden="true" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-b549f0e1e641"></use></svg>
        </button>
        ${tokens.map(t => {
        ...
        <button type="button" class="sb-icon-btn studio-rh-page-nav" data-plan-page-nav="next" title="Page suivante" aria-label="Page suivante"${nextDisabled}>
          <svg viewBox="0 0 24 24" aria-hidden="true" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-fb250e9a4a54"></use></svg>
        </button>
      </div>
    ...
    const rows = pageData.items.map(item => {
      ...
      return `
        <div class="studio-rh-table-row studio-rh-row--${esc(type)}">
          ...
          <div class="studio-rh-table-cell"><span class="ns-badge studio-rh-status">${esc(item.statut_label || statutLabel(item.statut))}</span></div>
  ...
  function modalIconSvg(type){
    ...
    return '<svg viewBox="0 0 24 24" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-c6819a8331b2"></use></svg>';
  }
  ...
  function addIndispoLine(startValue, endValue){
    ...
    row.innerHTML = `
      ...
      <button type="button" class="sb-icon-btn studio-rh-line-delete" data-indispo-remove="${esc(id)}" title="Supprimer la ligne" aria-label="Supprimer la ligne">
        <svg viewBox="0 0 24 24" aria-hidden="true" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-aa4824f9e286"></use></svg>
      </button>`;
    box.appendChild(row);
    updateIndispoLinesCount();
  }
