(function () {
    ...
    function buildOrgChartStatsText(stats){
        const s = stats || {};
        const parts = [
            `${parseInt(s.nb_services || 0, 10)} service(s)`,
            `${parseInt(s.nb_postes || 0, 10)} poste(s)`,
            `${parseInt(s.nb_collabs || 0, 10)} collaborateur(s)`,
        ];

        const nbNonLies = parseInt(s.nb_postes_non_lies || 0, 10);
        if (nbNonLies > 0){
            parts.push(`${nbNonLies} non lié(s)`);
        }

        return parts.join(" · ");
    }

    function buildOrgChartPostRow(poste, portal){
        const row = document.createElement("div");
        row.className = "sb-row-card";
        row.style.cursor = poste && poste.id_poste ? "pointer" : "default";
        ...
        code.textContent = (poste && poste.code) ? String(poste.code) : "—";

        const title = document.createElement("div");
        title.className = "sb-row-title";
        title.textContent = (poste && poste.intitule) ? String(poste.intitule) : "";
        ...
        badge.textContent = `${parseInt((poste && poste.nb_collabs) || 0, 10)} collab.`;
        right.appendChild(badge);

        row.appendChild(left);
        row.appendChild(right);

        if (poste && poste.id_poste){
            row.addEventListener("click", () => {
                try {
                    openEditPosteModal(portal, {
                        ...
                        id_service: poste.id_service || null,
                        nb_collabs: parseInt(poste.nb_collabs || 0, 10),
                        actif: true,
                    });
                } catch(_){}
            });
    ...
    function buildOrgChartServiceNode(service, depth, portal){
        const wrap = document.createElement("div");
        wrap.style.marginTop = depth > 0 ? "10px" : "0";

        if (depth > 0){
            wrap.style.marginLeft = `${Math.min(depth, 6) * 18}px`;
            wrap.style.paddingLeft = "14px";
            wrap.style.borderLeft = "2px solid rgba(17,24,39,.08)";
        }
        ...
        title.textContent = (service && service.nom_service) ? String(service.nom_service) : "Service";

        const sub = document.createElement("div");
        sub.className = "card-sub";
        sub.style.margin = "0";
        sub.textContent = "Service";
        ...
        meta.className = "sb-badge sb-badge--accent-soft";
        meta.textContent = `${parseInt((service && service.nb_postes) || 0, 10)} poste(s) · ${parseInt((service && service.nb_collabs) || 0, 10)} collab.`;
        ...
        const postes = Array.isArray(service?.postes) ? service.postes : [];
        if (postes.length){
            const posteHost = document.createElement("div");
            posteHost.style.display = "flex";
            posteHost.style.flexDirection = "column";
            posteHost.style.gap = "8px";
            posteHost.style.marginTop = "10px";

            postes.forEach(poste => {
                posteHost.appendChild(buildOrgChartPostRow(poste, portal));
            });

            card.appendChild(posteHost);
        }

        wrap.appendChild(card);

        const enfants = Array.isArray(service?.enfants) ? service.enfants : [];
        enfants.forEach(child => {
            wrap.appendChild(buildOrgChartServiceNode(child, depth + 1, portal));
        });
    ...
    function renderOrgChart(){
        const host = byId("orgChartTree");
        const summary = byId("orgChartSummary");
        if (!host || !summary) return;

        const data = _orgChartData || {};
        const stats = data.stats || {};
        const services = Array.isArray(data.services) ? data.services : [];
        const postesNonLies = Array.isArray(data.postes_non_lies) ? data.postes_non_lies : [];
        const portal = window.portal;

        summary.textContent = buildOrgChartStatsText(stats);
        host.innerHTML = "";

        if (!services.length && !postesNonLies.length){
        ...
        services.forEach(service => {
            host.appendChild(buildOrgChartServiceNode(service, 0, portal));
        });

        if (postesNonLies.length){
            const block = document.createElement("div");
            block.style.marginTop = services.length ? "14px" : "0";
            ...
            title.className = "card-title";
            title.style.marginBottom = "2px";
            title.textContent = "Postes non liés";

            const sub = document.createElement("div");
            sub.className = "card-sub";
            sub.style.margin = "0";
            sub.textContent = "Postes actifs sans rattachement à un service.";
            ...
            badge.textContent = `${postesNonLies.length} poste(s)`;
    ...
    async function openOrgChartModal(portal){
        const ownerId = getOwnerId();
        if (!ownerId) throw new Error("Owner manquant (?id=...).");

        const host = byId("orgChartTree");
        const summary = byId("orgChartSummary");
        if (host) host.innerHTML = `<div class="card-sub">Chargement de l’organigramme…</div>`;
        if (summary) summary.textContent = "—";

        openModal("modalOrgChart");

        const url = `${portal.apiBase}/studio/org/organigramme/${encodeURIComponent(ownerId)}`;
        _orgChartData = await portal.apiJson(url);
        renderOrgChart();
    }
    ...
    async function openOrgChartPdf(portal){
        ...
        if (!resp.ok){
            let msg = `Erreur export PDF (${resp.status})`;
            try{
                const err = await resp.json();
                if (err && err.detail) msg = String(err.detail);
            } catch(_){}
    ...
    function bindOnce(portal){
        ...
        byId("btnOpenOrgChart")?.addEventListener("click", async () => {
            try { await openOrgChartModal(portal); }
            catch (e) { portal.showAlert("error", e?.message || String(e)); }
        });

        byId("btnCloseOrgChart")?.addEventListener("click", () => closeOrgChartModal());
        byId("btnOrgChartClose")?.addEventListener("click", () => closeOrgChartModal());
        byId("btnOrgChartPdf")?.addEventListener("click", async () => {
            try { await openOrgChartPdf(portal); }
            catch (e) { portal.showAlert("error", e?.message || String(e)); }
        });

        const orgChartModal = byId("modalOrgChart");
        if (orgChartModal && !orgChartModal._sbBound){
            orgChartModal._sbBound = true;

            orgChartModal.addEventListener("click", (e) => {
                if (e.target === orgChartModal) closeOrgChartModal();
            });

            document.addEventListener("keydown", (e) => {
                const el = byId("modalOrgChart");
