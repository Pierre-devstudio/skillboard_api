...
def get_collaborateur_competence_fiche_pdf(
    id_contact: str,
    id_effectif: str,
    id_comp: str,
    request: Request,
    id_poste: Optional[str] = Query(default=None),
):
    """
    ...
    - si id_poste est fourni, la compétence peut aussi provenir du poste analysé
      (cas matching : compétence non encore détenue par le collaborateur).
    """
    try:
        comp_id = (id_comp or "").strip()
        poste_analyse_id = (id_poste or "").strip()
        if not comp_id:
            ...
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                cur.execute(
                    ...
                    WITH visible AS (
                        ...
                        SELECT fpc_match.id_competence AS id_comp
                        FROM public.tbl_fiche_poste_competence fpc_match
                        JOIN public.tbl_fiche_poste fp_match
                          ON fp_match.id_poste = fpc_match.id_poste
                        WHERE fpc_match.id_poste = %s
                          AND fp_match.id_ent = %s
                          AND COALESCE(fp_match.actif, TRUE) = TRUE
                          AND COALESCE(fpc_match.masque, FALSE) = FALSE
                    )
                    SELECT
                        c.id_comp,
                        c.code,
                    ...
                    LIMIT 1
                    """,
                    (id_poste, id_effectif, poste_analyse_id or None, id_ent, comp_id),
                )
                row = cur.fetchone() or {}
                if not row:
                    raise HTTPException(status_code=404, detail="Compétence introuvable pour ce collaborateur ou ce poste analysé.")
