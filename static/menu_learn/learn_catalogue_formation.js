(function () {
  ...
  function closeModal(id){
    ...
    function iconHtml(){
        return `
            <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-save"></use></svg>
        `;
    }
...
function iconPdf(){
  ...
  function iconPlus(){
    return `
      <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-858f27a56bec"></use></svg>
    `;
  }

  function iconSync(){
    return `
      <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-805767d51ea1"></use></svg>
    `;
  }

    function iconDoubleUp(){
    return `
        <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-a7b2ded27505"></use></svg>
    `;
    }
  ...
  function renderRefChecks(){
    ...
    function findCompDomainIdFromLabel(label){
        const raw = normalizeImportKey(label || "");
        if (!raw) return "";
        ...
        (_compDomainItems || []).forEach(d => {
            ...
            const blob = [d.titre_court, d.titre, d.description, id]
                .map(x => normalizeImportKey(x || ""))
                .filter(Boolean)
                .join(" ");
            ...
            let score = 0;
            if (blob === raw) score = 100;
            else if (blob.includes(raw) || raw.includes(blob)) score = 88;
            else {
                const rawTokens = new Set(raw.split(" ").filter(x => x.length > 2));
                const blobTokens = new Set(blob.split(" ").filter(x => x.length > 2));
                const common = Array.from(rawTokens).filter(x => blobTokens.has(x)).length;
                const denom = Math.max(1, rawTokens.size + blobTokens.size - common);
    ...
    async function openPendingCompetenceCreate(target, idx, proposal){
        ...
        const p = proposal || {};
        const title = (p.source || p.intitule || "").trim();
        const description = (p.description || "").trim();
        const domaineId = (p.domaine_id || "").toString().trim();
        ...
        if (byId("formCompNivD")) byId("formCompNivD").value = "";

        fillCompDomainSelect(domaineId);
        fillCompAiDomainSelect(domaineId);
    ...
    function ensureCompAiDocumentUpload(){
        ...
        drop.innerHTML = `
            <div class="lf-comp-ai-doc-icon" aria-hidden="true">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-legacy-05ef3e58f96d"></use></svg>
            </div>
      ...
      card.innerHTML = `
        <div class="lf-prereq-card-head">
          ...
          <button type="button" class="sb-icon-btn sb-icon-btn--danger" title="Retirer" aria-label="Retirer">
            <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ns-icon-use"><use href="/novoskill_icons.svg#ns-icon-info"></use></svg>
          </button>
        </div>
  ...
  function fillFormationModal(d){
    ...
    function importRowsToCreate(rows){
        return (rows || [])
            ...
            .map(r => ({
            source: (r.source || r.intitule || "").toString().trim(),
            description: (r.description || "").toString().trim(),
            domaine_hint: (r.domaine_hint || r.domaine_label || "").toString().trim(),
