(function () {
  ...
  async function initAuth() {
    if (window.__peopleAuthReady) return window.__peopleAuthReady;

    window.__peopleAuthReady = (async () => {
    ...
    try {
      return await window.__peopleAuthReady;
    } catch (e) {
      window.__peopleAuthReady = null;
      throw e;
    }
  ...
  async function ensurePortalEntry() {
    ...
    const loginUrl = (window.PORTAL_LOGIN_URL || "/people_login.html");
  ...
  async function wireLogout() {
    ...
    btn.addEventListener("click", async () => {
      try {
        ...
        btn.disabled = false;
        window.location.href = (window.PORTAL_LOGIN_URL || "/people_login.html");
      }
    });
  }
