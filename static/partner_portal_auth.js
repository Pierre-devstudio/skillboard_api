(function () {
  ...
  window.__partnerAuthReady = null;
  ...
  async function initAuth() {
    if (window.__partnerAuthReady) return window.__partnerAuthReady;

    window.__partnerAuthReady = (async () => {
    ...
    try {
      return await window.__partnerAuthReady;
    } catch (e) {
      window.__partnerAuthReady = null;
      throw e;
    }
  ...
  async function ensurePortalEntry() {
    ...
    const loginUrl = (window.PORTAL_LOGIN_URL || "/partner_login.html");
    ...
    try { consultantId = window.PortalAuthCommon.getContactId(); } catch (_) {}

    if (!consultantId) {
      try { consultantId = await window.PortalAuthCommon.ensureContactIdFromSession(); } catch (_) {}
    }
    ...
    window.location.href = buildPortalUrlWithId(consultantId);
  }
  ...
  async function wireLogout() {
    ...
    btn.addEventListener("click", async () => {
      try {
        ...
        btn.disabled = false;
        window.location.href = (window.PORTAL_LOGIN_URL || "/partner_login.html");
      }
    });
  }
